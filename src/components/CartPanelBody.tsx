...
interface Props {
  // Renders a header close (X) button when provided — used by the mobile
  // drawer. The desktop dock omits it since it's a persistent panel.
  onClose?: () => void;
}

export default function CartPanelBody({ onClose }: Props) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid rgba(26,16,8,0.1)', background: 'var(--suya-smoke)' }}>
        ...
        {onClose && (
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ash-white)' }}>
            <X size={22} />
          </button>
        )}
      </div>
